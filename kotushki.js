async function fetchProductCategoryDetails() {
    try {
        const response = await fetch(`https://localhost:7118/api/Product/GetProductCategoryDetail?Id=${1}`);
        if (!response.ok) {
        ...
        categoryDetails.forEach(detail => {
            ...
            detailBox.innerText = detail.categoryDetail; // Assuming `detail` is the correct property name
